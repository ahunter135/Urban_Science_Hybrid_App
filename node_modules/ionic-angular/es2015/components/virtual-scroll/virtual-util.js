...
export function populateNodeData(startCellIndex, endCellIndex, scrollingDown, cells, records, nodes, viewContainer, itmTmp, hdrTmp, ftrTmp) {
    ...
    const /** @type {?} */ unusedNodes = nodes.filter(n => usedNodes.indexOf(n) < 0);
    unusedNodes.forEach(node => {
        const /** @type {?} */ index = viewContainer.indexOf(node.view);
        viewContainer.remove(index);
        const /** @type {?} */ removeIndex = nodes.findIndex(n => n === node);
...
export function adjustRendered(cells, data) {
    const /** @type {?} */ maxRenderHeight = (data.renderHeight - data.itmHeight);
    const /** @type {?} */ totalCells = cells.length;
    const /** @type {?} */ viewableRenderedPadding = (data.itmHeight < 90 ? VIEWABLE_RENDERED_PADDING : 0);
    if (data.scrollDiff > 0) {
        ...
        for (let /** @type {?} */ i = data.topCell; i < totalCells; i++) {
        ...
        if (cellsRenderHeight < maxRenderHeight) {
            ...
            for (let /** @type {?} */ i = data.topCell - 1; i >= 0; i--) {
                cellsRenderHeight += cells[i].height;
                data.topCell = i;
                if (cellsRenderHeight >= maxRenderHeight)
                    break;
        ...
        if (cellsRenderHeight < maxRenderHeight) {
            ...
            for (let /** @type {?} */ i = data.bottomCell; i < totalCells; i++) {
                cellsRenderHeight += cells[i].height;
                data.bottomCell = i;
                if (cellsRenderHeight >= maxRenderHeight)
                    break;
